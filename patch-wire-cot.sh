...
# 1) Write/overwrite the stdout flattener+logger
...
# 2) Ensure we import + call the installer in app bootstrap
APP=src/orchestration/app.ts
if ! grep -q 'installStdoutThinkFlatten' "$APP" 2>/dev/null; then
  # import at very top
  sed -i '1i import { installStdoutThinkFlatten } from "../core/utils/stdout-think-flatten";' "$APP"
fi

# Insert call immediately after the import block (first non-import line)
awk '
  BEGIN{inserted=0}
  {
    if(!inserted && $0 !~ /^import /){
      print "installStdoutThinkFlatten(); // SHOW_THINK=1 flattens CoT; DEBUG_COT=1 logs raw bytes";
      inserted=1
    }
    print
  }
  END{ if(!inserted) print "installStdoutThinkFlatten();" }
' "$APP" > "$APP.tmp" && mv "$APP.tmp" "$APP"

git add -A
git commit -m "wire(stdout): installStdoutThinkFlatten + raw byte logger (DEBUG_COT) and call it at bootstrap" || true
