...
// Mock "f-server": responds deterministically with two tools then final text.
// Same protocol as the other mock but returns "done(h8)" on completion.
function startFServer(port: number = 0) {
  ...
  const server = http.createServer(async (req: any, res: any) => {
    ...
    req.on("end", () => {
      ...
      let message: any;
      if (toolCount < 1) {
        message = { role: "assistant", content: "", tool_calls: [toolCall("echo left")] };
      } else if (toolCount < 2) {
        message = { role: "assistant", content: "", tool_calls: [toolCall("echo right")] };
      } else {
        message = { role: "assistant", content: "done(h8)" };
      }
...
test("routes via OpenAI base, then emits @group", async () => {
  ...
  const server = await startFServer(0);
