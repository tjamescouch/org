...
export class PodmanSession implements ISandboxSession {
    ...
    async start() {
        ...
        const mounts = [
            ...
            "-v", `${this.spec.workHostDir}:/work:rw,z,U`,
        ];

        // Do NOT set "-w /work" here (workdir checked before mounts exist).
        await this.execHost(this.tool, [
            "create", "--name", this.name, ...netArgs, ...caps, ...limits, ...mounts,
            this.spec.image, "sleep", "infinity",
        ]);
        await this.execHost(this.tool, ["start", this.name]);

        // Prime /work from /project, but PROTECT .org/ from deletion.
        // Then ensure runner + steps dir exist on the writable mount.
        ...
        // Create baseline commit for diffing.
        await this.execInCmd("git -C /work init && git -C /work config user.email noreply@example && git -C /work config user.name org && git -C /work add -A && git -C /work commit -m baseline >/dev/null");
    ...
    async exec(cmd: string): Promise<{ ok: boolean; exit: number; stdoutFile: string; stderrFile: string }> {
        ...
        // --- run the step runner with ENV (must be BEFORE the container name) ---
        ...
        const r = await this.execInEnv(env, `/work/.org/org-step.sh ${this.shQ(cmd)}`);
        ...
        const show = await this.execInCmd("git -C /work diff --name-only HEAD~1..HEAD || true");
        ...
        return { ok: r.code === 0, exit: r.code, stdoutFile: hostOut, stderrFile: hostErr };
    }

    async finalize(): Promise<{ manifestPath: string; patchPath?: string }> {
        if (!this.started) throw new Error("session not started");
        ...
        const listNew = await this.execInCmd("git -C /work diff --name-status " + this.shQ(this.baselineCommit!) + " HEAD | awk '$1 ~ /^A|^AM$/ {print $2}'");
