...
type ExecOpts = {
    ...
    streamToTty?: boolean; // if you want to live-echo output
};

function run(cmd: string, args: string[], opts: ExecOpts = {}) {
    return new Promise<{ code: number; stdout: string; stderr: string }>((resolve) => {
        const child = spawn(cmd, args, {
            cwd: opts.workdir,
            env: { ...process.env, ...opts.env },
            stdio: opts.streamToTty ? ["pipe", "inherit", "inherit"] : ["pipe", "pipe", "pipe"],
        });
...
export class PodmanSession implements ISandboxSession {
    ...
    async start() {
        ...
        // Baseline commit (always ensure HEAD exists)
        await this.must(
            this.execInCmd(
                "git -C /work add -A && " +
        ...
        // Verify and record HEAD
        const rev = await this.execInCmd("git -C /work rev-parse --verify HEAD");
        await this.must(rev, "git rev-parse HEAD");
